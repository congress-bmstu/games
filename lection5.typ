        ...
        cetz.canvas(
          {
            ...
            content("plot.U", $U$)
            line("plot.O", "plot.a")
            mark("plot.a", "plot.at", symbol: ">", fill: black)
          },
          length: 50%,
        ),
      )
...
#example[
  ...
  Перейдем в пространство критериев следующим преобразованием $arrow(f)=F arrow(x)$:
  $ F=mat(6,9;2,1;) $
  ...
  Ищем точку ближайшую к идеальной $f^*, x^*$. Она лежит на пересечении BC и перпендикуляра к BC, проходящим через точку $I$.

  Уравнение прямой BC $ (f_2-16)/(-5)=(f_1-48)/15 => f_1+3f_2=96. $<lec5_bc>

  Уравнение нормали к BC проходящей через точку $f^*$ и $I$ легко составляется из уравнения @lec5_bc (меняем коэфы местами и докидываем минус у $f_2$). Получается $3f_1-f_2$, правую часть посчитаем, подставив координаты точки $I$, получим $3f_1-f_2=173$.
  ...
  Вернемся назад в пространство параметров $arrow(x)=F^(-1)arrow(f)$:
